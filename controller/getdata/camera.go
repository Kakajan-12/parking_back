...
//	carData.CarNumber = capturedData.EventComment
//	carData.Status = statusInside
//	carData.StartTime = now
//	carData.ImageUrl = defaultImageURL
//	carData.Reason = "entry"
//	carData.PayStatus = true
//	var existingCar modelscar.CarModel
//	err := database.DB.Order("id desc").First(&existingCar,
//		"car_number = ? AND (status = ? OR status = ?)",
//		carData.CarNumber, statusInside, statusPending).Error
//
//	if err == nil {
...
//// @Router /api/v1/camera/get-data [put]
...
//	}
//
//	startTimeStr := carData.StartTime
...
//
//	if minutes <= 360 {
//		carData.TotalPayment = 2
//	} else if minutes <= 1440 {
//		carData.TotalPayment = 3
//	} else {
//		days := int(math.Ceil(minutes / 1440))
//		carData.TotalPayment = float64(3 * days)
//	}
//
//	if util.IsVIPPlate(capturedData.EventComment) {
//		carData.TotalPayment = 0
//	}
//	carData.Status = statusPending
//	carData.EndTime = endTimeStr
...
//	}
//
//	carData.CameraToken = capturedData.ChannelID
//
//	ip := os.Getenv("HOST")
//	port := os.Getenv("PORT")
//	carData.ImageUrl = fmt.Sprintf("http://%s:%s/plate/%s", ip, port, carData.ImageUrl)
//
...
//}
//
//// CreateCarExitNoWs handles the car exit process from the parking lot
//// @Summary Create a car exit record in the parking lot
...
//// @Failure 404 {object} resmodel.ErrorResponse "Car not found"
//// @Failure 500 {object} resmodel.ErrorResponse "Internal server error, failed to update data"
//// @Router /api/v1/camera/get-data/realtime [put]
